...
-- Fix: Convert ALL RESTRICTIVE policies to PERMISSIVE (OR logic)
...
CREATE POLICY "Managers can view all profiles" ON public.profiles FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'manager'::app_role));
CREATE POLICY "Users can view their own profile" ON public.profiles FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own profile" ON public.profiles FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own profile" ON public.profiles FOR UPDATE TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Managers can update any profile" ON public.profiles FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'manager'::app_role));
...
CREATE POLICY "No direct role updates allowed" ON public.user_roles FOR UPDATE TO authenticated USING (false);
CREATE POLICY "No direct role deletes allowed" ON public.user_roles FOR DELETE TO authenticated USING (false);
...
CREATE POLICY "Users can mark received messages as read" ON public.messages FOR UPDATE TO authenticated USING (auth.uid() = receiver_id) WITH CHECK (is_read = true);
...
CREATE POLICY "Anyone can view society settings" ON public.society_settings FOR SELECT TO authenticated USING (true);
CREATE POLICY "Managers can update society settings" ON public.society_settings FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'manager'::app_role));
CREATE POLICY "Managers can insert society settings" ON public.society_settings FOR INSERT TO authenticated WITH CHECK (public.has_role(auth.uid(), 'manager'::app_role));
CREATE POLICY "No one can delete society settings" ON public.society_settings FOR DELETE TO authenticated USING (false);

-- Seed default settings row
INSERT INTO public.society_settings (name) SELECT 'My Housing Society' WHERE NOT EXISTS (SELECT 1 FROM public.society_settings LIMIT 1);
